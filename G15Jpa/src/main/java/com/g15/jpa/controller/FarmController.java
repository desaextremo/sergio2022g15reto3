...
public class FarmController {
    ...
    @PostMapping("/save")
    public ResponseEntity insertFarm(@RequestBody Farm finca){
       farmService.insertFarm(finca);
       return ResponseEntity.status(201).build();
    }
    
    @PutMapping("/update")
    public ResponseEntity updateFarm(@RequestBody Farm finca){
       farmService.updateFarm(finca);
       return ResponseEntity.status(201).build(); 
    }
